...
-->
	<cfif IsDefined("URL.reply")>
    	<cfquery name="ReplyMessage" datasource="webwarecl">
...
<div style="height:40px;">
	...
	<a href="##" class="button" onclick="viewMailFolder('inbox')"><span>My Inbox</span></a>   
</div>
                ...
                <cfelse>												<!--- NEW MESSAGE --->
                	<cfif URL.FrameworkRevision LT 2.0>					<!--- FW 1.x --->
						<cfif IsDefined("url.toID")> 
							...
							<cfset usrFound = false>
							<cfoutput query="usr">
								<option value="#target_id#" 
										<cfif target_id EQ url.toID>
											selected
											<cfset usrFound = true>
										</cfif>>#getLongName(target_id)#</option>
							</cfoutput>
							<cfif NOT usrFound>
								<cfoutput>
								<option value="#url.toID#" selected>#getLongName(url.toID)#</option>
								</cfoutput>
							</cfif>
							</select>
						<cfelse>										<!--- FW 2.0+ --->
