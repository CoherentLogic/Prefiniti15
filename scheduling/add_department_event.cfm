...
<div style="width:100%; background:url(/graphics/binary-bg.jpg); background-repeat:no-repeat; height:80px; border-bottom:2px solid #EFEFEF; background-color:white;">
	<div style="float:left">
    	<h3 class="stdHeader" style="padding:10px;"><img src="/graphics/globe-compass-48x48.png" align="top"> Add Department Event</h3>
    </div>
</div>
...
<table width="100%" cellspacing="0" cellpadding="5" style="clear:both;">
    ...
    <tr>
        ...
        <!--scCreateEvent(user_id, date, start_block, end_block, event_description,
        	...
        	<cfoutput><input type="button" class="normalButton" value="Save Event" onclick="scCreateDepartmentEvent(#url.department_id#, GetValue('date'), GetValue('start_block'), GetValue('end_block'), GetValue('event_description'), GetValue('event_title'), GetValue('address'), GetValue('city'), GetValue('state'), GetValue('zip'));"></cfoutput>
		</td>
	</tr>                    
</table>
