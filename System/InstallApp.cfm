...
<body>
    ...
    <cfset thisApp = createObject("component","System.App").Open(URL.AppID)>
    <cfoutput>
	<div style="margin-left:80px; margin-right:90px;">
    <p><img src="#thisApp.Icon#" align="absmiddle" /> The <strong>#thisApp.AppName#</strong> app must be installed to your <strong>#session.User.LoginSession.ActiveMembership.MembershipType#</strong> membership on the <strong>#session.User.LoginSession.ActiveMembership.SiteName#</strong> site in order to function in this Prefiniti session.</p>
	...
	<input type="button" value="Install App" onclick="location.replace('/System/InstallApp.cfm?AppID=#URL.AppID#&DoInstall');"/>
	</cfoutput>
    </div>
</body>
</html>
<cfelse>
	<cfscript>
		thisApp = createObject("component", "System.App").Open(URL.AppID);
		thisApp.Install(session.User.LoginSession.ActiveMembership);
	</cfscript>
    
    <cflocation url="/System/default.cfm?AppID=#URL.AppID#&View=FirstRun" addtoken="no">
</cfif>
