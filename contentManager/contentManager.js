...
{
	/*
	commented out for new windowed interface on 3-26-09 by AG
	var url;
	...
	var AF_handle = 'Associate-File';
	var wRef = p_session.Framework.FindWindow(AF_handle);
	if (!wRef) {
		
		var AF_title  = 'Associate File';
		var AF_icon   = new PIcon('/graphics/folder.png', P_SMALLICON);
		var AF_rect   = new PAutoRect(368, 220);
		var AF_style  = WS_ALLOWCLOSE;
		var AF_MessageHandler  = onAssociateLoad;
...
function onAssociateLoad(handle, msgid, sender_component, msg_object)
{
	p_session.Framework.DefaultMessageHandler(handle, msgid, sender_component, msg_object);
}
function cmsAddAssociation(file_id, project_id, scope, description, releasable)
{
	...
	var onRequestCallback = function() {
		p_session.Framework.PostLocalMessage("Associate-File", IWC_REQUESTCLOSE, C_WINDOWMANAGER, null);
		AjaxRefreshTarget();
	};
